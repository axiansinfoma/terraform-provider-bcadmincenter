...
func BuildAuthorizedEntraAppID(tenantID, appID string) string {
	return fmt.Sprintf("/tenants/%s/providers/%s/authorizedEntraApps/%s",
		tenantID, ProviderNamespace, appID)
}
...
func ParseAuthorizedEntraAppID(id string) (string, string, error) {
	...
	if len(parts) != 6 {
		return "", "", fmt.Errorf("invalid authorized Entra app ID format: expected '/tenants/{tenantId}/providers/%s/authorizedEntraApps/{appId}', got: %s", ProviderNamespace, id)
	}
	...
	if parts[3] != ProviderNamespace {
		return "", "", fmt.Errorf("invalid authorized Entra app ID: expected provider namespace '%s', got: %s", ProviderNamespace, parts[3])
	}
