...
// BuildUpdateScheduleID creates an ARM-like resource ID for an environment update schedule.
// Format: /tenants/{tenantId}/providers/Microsoft.Dynamics365.BusinessCentral/applications/{applicationFamily}/environments/{environmentName}/updateSchedule.
func BuildUpdateScheduleID(tenantID, applicationFamily, environmentName string) string {
return fmt.Sprintf("/tenants/%s/providers/%s/applications/%s/environments/%s/updateSchedule",
tenantID, constants.ProviderNamespace, applicationFamily, environmentName)
}

// ParseUpdateScheduleID parses an environment update schedule resource ID.
// Returns: tenantID, applicationFamily, environmentName, error.
func ParseUpdateScheduleID(id string) (string, string, string, error) {
parts := strings.Split(strings.TrimPrefix(id, "/"), "/")

// Expected format: tenants/{tenantId}/providers/Microsoft.Dynamics365.BusinessCentral/applications/{applicationFamily}/environments/{environmentName}/updateSchedule.
if len(parts) != 9 {
return "", "", "", fmt.Errorf("invalid update schedule ID format: expected '/tenants/{tenantId}/providers/%s/applications/{applicationFamily}/environments/{environmentName}/updateSchedule', got: %s", constants.ProviderNamespace, id)
}

if parts[0] != "tenants" {
return "", "", "", fmt.Errorf("invalid update schedule ID: expected 'tenants' segment, got: %s", parts[0])
}

if parts[2] != "providers" {
return "", "", "", fmt.Errorf("invalid update schedule ID: expected 'providers' segment, got: %s", parts[2])
}

if parts[3] != constants.ProviderNamespace {
return "", "", "", fmt.Errorf("invalid update schedule ID: expected provider namespace '%s', got: %s", constants.ProviderNamespace, parts[3])
}

if parts[4] != "applications" {
return "", "", "", fmt.Errorf("invalid update schedule ID: expected 'applications' segment, got: %s", parts[4])
}

if parts[6] != "environments" {
return "", "", "", fmt.Errorf("invalid update schedule ID: expected 'environments' segment, got: %s", parts[6])
}

if parts[8] != "updateSchedule" {
return "", "", "", fmt.Errorf("invalid update schedule ID: expected 'updateSchedule' segment, got: %s", parts[8])
}
